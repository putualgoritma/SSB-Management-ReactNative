...
import { Button, Footer, Header2, Search, Spinner,Title,ButtonAdd } from '../../../component';
import API from '../../../service';
import { colors } from '../../../utils/colors';
import Distance from '../../../utils/distance';

const Periode =({navigation})=>{

  const TOKEN = useSelector((state) => state.TokenReducer);
  const [dataPeriode, setDataPeriode] = useState(null);
  const [loading, setLoading] = useState(true);
  const [filterPeriode, setFilterPeriode] = useState('')
  const tableHead = ['NO', 'Periode', 'Aksi'];
        ...
        data[index]= [
          ...
          <View style={{padding : 4}} >
            <TouchableOpacity style={{backgroundColor : colors.button, height : 30, justifyContent : 'center'}}
              onPress={() => navigation.navigate('DetailPeriode', {periods : item})}
            >
            ...
            onPress={() => navigation.navigate('EditPeriode')}
              >
  ...
  const handleFilter = () => {
    if(filterPeriode !== ''){
      ...
      dataPeriode.map((item, index) => {
        ...
        if(seacrh.includes(filterPeriode.name.toLowerCase())){
          no[index] = index + 1;
          data[index]= [
            item.name,
            <View style={{padding : 4}} >
              <TouchableOpacity style={{backgroundColor : colors.button, height : 30, justifyContent : 'center'}}
                onPress={() => navigation.navigate('DetailPeriode', {grade : item})}
              >
                <Text style={{textAlign:"center", color : '#ffffff'}} >Details</Text>
              </TouchableOpacity>
              <Distance distance={3}/>
       ...
       {!loading ? null : 
        ...
        <View style={styles.section}>
          ...
          <View style={{paddingHorizontal : 20}}>
              {/* <Text style={styles.titlePage} >Periode</Text> */}
              <Title title="Periode"/>
              <Distance distance = {5}/>
              <View style={{flexDirection:'row'}}>
                <Search
                  onChangeText = {(value) => setFilterPeriode({...filterPeriode, name : value})}
                />
                <Distance distanceH = {5}/>
                <Button title="Filter" onPress = {handleFilter} width="27%"/>
              </View>
              <Distance distance = {8}/>
              <ButtonAdd onPress={() =>navigation.navigate('AddPeriode')}/>
              <Distance distance ={8}/>
